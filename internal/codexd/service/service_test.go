...
func TestRetentionCount(t *testing.T) {
	...
	id1 := startExec(t, h, `echo one`)
	id2 := startExec(t, h, `echo two`)
	id3 := startExec(t, h, `echo three`)
	_ = id1
	_ = id2
	_ = id3
	...
	for time.Now().Before(deadline) {
		n = countExecDirs(t, filepath.Join(dir, "exec"))
		if n <= 2 {
			ok = true
			break
		}
...
func countExecDirs(t *testing.T, execRoot string) int {
	t.Helper()
...
func TestExecRunPersistsArtifacts(t *testing.T) {
	...
	if strings.TrimSpace(string(exitCode)) != "0" {
		t.Fatalf("exit_code = %q, want 0", strings.TrimSpace(string(exitCode)))
	}
}
